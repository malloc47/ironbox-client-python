...
class IronBoxRESTClient():
    ...
    def __init__(self, entity, entity_password, entity_type=0, version='latest', content_format='application/json',
                 verbose=False, verifySSLCert=True):
            ...
            for chunk in r.iter_content(chunk_size=1024):
                if chunk:
                    ...
                    if self.Verbose is True:
                        sys.stdout.write(
                            "\rDownloaded %d encrypted byte(s) to %s" % (numBytesDownloaded, EncryptedFilePath))
                        sys.stdout.flush()
                    f.flush()
